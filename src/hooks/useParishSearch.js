...
export const useParishSearch = (mapElement) => {
  ...
  const zoomToFeature = useCallback(
    async (whereClause) => {
      ...
      try {
        ...
        const extentResult = await targetLayer.queryExtent({
          where: whereClause,
        });
        if (extentResult?.extent) {
          // Highlight the feature
          if (highlightManagerRef.current) {
            await highlightManagerRef.current.highlightFeature(
              targetLayer,
              whereClause,
            );
          }
  ...
  const shouldZoom = useCallback((newSelection) => {
    return (
      JSON.stringify(newSelection) !== JSON.stringify(lastSelection.current)
    );
  }, []);
  ...
  const handleComboBoxSelect = useCallback(
    async (county = null, municipality = null, parish = null) => {
      const newSelection = {
        county: county || "",
    ...
    [zoomToFeature, shouldZoom, handleSearchClear],
  );
  ...
  const queryParishLayer = useCallback(
    async (geometry) => {
