...
export const useMapData = (mapElement) => {
  ...
  const fetchUniqueValues = useCallback(
    async (layer, field, whereClause = null) => {
      try {
  ...
  useEffect(() => {
    ...
    if (mapElement.ready && mapElement.map) {
      initializeData();
      return;
    }
    ...
    mapElement.addEventListener("arcgisViewReadyChange", handleReady);

    return () => {
      mapElement.removeEventListener("arcgisViewReadyChange", handleReady);
    };
  }, [mapElement, initializeData]);
