  ...
  const lowerLinks = [
    {
      ...
      links: [
        {
          label: "Digital Norway Parish Map",
          path: "https://norwayparishes.com/",
        },
        {
          label: "Immigration Books (PDFs)",
          path: "https://www.nagcnl.org/immigration-books-pdfs/",
        },
        {
          ...
          links: [
            {
              label: "Civil War Soldiers",
              path: "https://www.nagcnl.org/databases/#soldiers",
            },
            {
              label: "Birth Records",
              path: "https://www.nagcnl.org/databases/#birth",
            },
            {
              label: "Marriage Records",
              path: "https://www.nagcnl.org/databases/#marriage",
            },
            {
              label: "Cemetery Records",
              path: "https://www.nagcnl.org/databases/#cemetery",
            },
            {
              label: "Immigration",
              path: "https://www.nagcnl.org/databases/#immigration",
            },
            {
              label: "Census",
              path: "https://www.nagcnl.org/databases/#census",
            },
          ],
        },
      ],
  ...
  return (
    <div className="bg-nagcnl-green">
      ...
      <nav className="bg-nagcnl-green px-[20px] bs-xxl:px-[45px] py-[15px] mx-[15px] bs-lg:mx-[30px] bs-xl:mx-[75px] bs-xxl:mx-[100px] relative">
          ...
          <ul className="hidden bs-xl:flex flex-1 justify-center">
            {upperLinks.map((link, idx) => (
              <li className="px-[10px]" key={idx}>
                <a
                  href={link.path}
                  className="text-nagcnl-white font-lora text-[16px] bs-xxxl:px-[25px] whitespace-nowrap"
                >
          ...
          <div className="flex items-center">
            <div className="hidden bs-md:flex items-center">
              ...
              <div className="nagcnl-input-group ml-[15px] flex items-center">
                ...
                >
                  <i
                    className="fas fa-shopping-cart text-[18px] text-nagcnl-green hover:text-nagcnl-white"
                    aria-label="Cart"
                  ></i>
                </a>
              </div>
        ...
        <div className="hidden bs-xl:block mt-[15px]">
          <ul className="flex justify-around">
            {lowerLinks.map((link, idx) => (
              <li
                key={idx}
                className={`relative ${activeDropdown === idx ? "open" : ""}`}
              >
                <a
                  ...
                  className={`${
                    activeDropdown === idx
                      ? "text-nagcnl-green bg-nagcnl-white"
                      : "text-nagcnl-white rounded-b-[5px]"
                  } font-opensans text-[18px] font-bold inline-block cursor-pointer px-[20px] py-[8px] rounded-t-[5px]`}
                >
                  {link.label}
                  {link.links && (
                    <i
                      className={`fa-solid ${
                        activeDropdown === idx ? "fa-angle-up" : "fa-angle-down"
                      } ml-[10px]`}
                    />
                  )}
                </a>
        ...
        {menuOpen && (
          <div
            className="bs-xl:hidden bg-nagcnl-green border-t-[1px] border-nagcnl-white text-nagcnl-black absolute top-full left-0 w-full shadow-lg z-40 px-[30px] py-[30px]"
            aria-hidden="true"
          >
            <ul className="flex flex-col">
              {lowerLinks.map((link, idx) => (
                <li key={idx} className="flex flex-wrap">
                  ...
                  {link.links && (
                    <span onClick={() => toggleMobileLink(idx)}>
                      <i
                        className={`fa-solid ${
                          mobileOpenIndexes.includes(idx)
                            ? "fa-angle-up"
                            : "fa-angle-down"
                        } text-nagcnl-white cursor-pointer ml-[10px]`}
                      />
                    </span>
                  )}
            ...
            <ul className="flex mt-[30px]">
              {upperLinks.map((link, idx) => (
                <li className="px-[10px] flex-1" key={idx}>
                  <a
                    href={link.path}
                    className="text-nagcnl-white font-lora text-[16px] whitespace-nowrap flex flex-col items-center"
                  >
                    <i className={`fas ${link.icon} text-[14px] mr-[10px]`}></i>
